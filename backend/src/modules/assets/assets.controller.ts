...
export class AssetsController {
  ...
  @Post() create(@Request() req: any, @Body() dto: CreateAssetDto) { return this.service.create(req.user.userId, dto); }
  @Get(':id') findOne(@Request() req: any, @Param('id') id: string) { return this.service.findById(id, req.user.userId); }
  @Patch(':id') update(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateAssetDto) { return this.service.update(id, req.user.userId, dto); }
