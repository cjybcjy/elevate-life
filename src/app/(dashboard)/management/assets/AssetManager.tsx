...
export function AssetCurrencySelect() {
  return (
    <div className="min-w-44">
      <label htmlFor="asset-currency" className="mb-1 block text-xs text-ledger-muted">币种</label>
      <select
        id="asset-currency"
        name="currency"
        aria-label="币种"
        defaultValue="CNY"
        className="min-h-11 w-full rounded-md border border-ledger-bg bg-ledger-bg px-3 py-2 text-sm focus:border-ledger-accent focus:outline-none"
      >
        {assetCurrencyOptions.map((currency) => (
          <option key={currency.value} value={currency.value}>
            {currency.name}（{currency.value} {currency.symbol}）
          </option>
        ))}
      </select>
    </div>
