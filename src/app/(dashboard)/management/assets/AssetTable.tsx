...
export function AssetTable({
  ...
  const hasMobileVisibleAssets = assets.some((asset) => asset.category !== 'stock');
    ...
    <div className="rounded-xl bg-ledger-surface overflow-hidden overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {assets.length > 0 && !hasMobileVisibleAssets && (
            <tr className="md:hidden">
              <td colSpan={10} className="px-4 py-8 text-center text-ledger-muted">暂无非股票资产</td>
            </tr>
          )}
          {assets.map((asset: any) => {
            ...
            return (
              <tr
                key={asset.id}
                className={`border-b border-ledger-bg last:border-0 ${asset.category === 'stock' ? 'hidden md:table-row' : ''} ${isEditing ? 'bg-ledger-bg/30' : ''}`}
              >
                <td className="px-4 py-3" style={{ color: 'var(--color-text-primary)' }}>{asset.name}</td>
                    ...
                    <div className="mt-3 p-3 bg-ledger-bg rounded-lg border border-ledger-primary/20">
                      <form action={handleUpdate} className="flex flex-wrap gap-2 items-end">
                        ...
                        <div>
                          ...
                          <input name="name" defaultValue={asset.name} className="rounded-md bg-ledger-surface border border-ledger-bg px-2 py-1.5 text-xs focus:outline-none focus:border-ledger-accent w-24" />
                        </div>
                        ...
                        {asset.category === 'stock' || asset.category === 'fund' ? (
                          <>
                            <div>
                              ...
                              <select name="market" defaultValue={asset.market || ''} className="rounded-md bg-ledger-surface border border-ledger-bg px-2 py-1.5 text-xs focus:outline-none focus:border-ledger-accent">
                            ...
                            <div>
                              ...
                              <input name="stockCode" defaultValue={asset.stockCode || ''} className="rounded-md bg-ledger-surface border border-ledger-bg px-2 py-1.5 text-xs font-mono focus:outline-none focus:border-ledger-accent w-20" />
                            </div>
                          </>
                        ) : (
                          <div>
                        ...
                        {showDepreciation && (
                          <>
                            <div>
                              ...
                              <input name="purchaseDate" type="date" defaultValue={asset.purchaseDate ? new Date(asset.purchaseDate).toISOString().slice(0,10) : ''} className="rounded-md bg-ledger-surface border border-ledger-bg px-2 py-1.5 text-xs focus:outline-none focus:border-ledger-accent" />
                            </div>
                            <div>
                              <label className="block text-xs text-ledger-muted mb-1">报废日期</label>
                              <input name="scrapDate" type="date" defaultValue={asset.scrapDate ? new Date(asset.scrapDate).toISOString().slice(0,10) : ''} className="rounded-md bg-ledger-surface border border-ledger-bg px-2 py-1.5 text-xs focus:outline-none focus:border-ledger-accent" />
                            </div>
                            <div>
