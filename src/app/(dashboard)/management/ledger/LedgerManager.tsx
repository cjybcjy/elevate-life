...
const typeLabel = (type: string) => {
  switch (type) {
    ...
    case 'LIABILITY_DRAW': return '循环贷借入';
    default: return type;
  }
};
...
const typeBadgeClass = (type: string) => {
  switch (type) {
    ...
    case 'LIABILITY_DRAW': return 'bg-amber-900/50 text-amber-300';
...
export default function LedgerManager({ view = 'transactions' }: { view?: LedgerView }) {
  ...
  useMobileEditorScroll(editingTxId ? `transaction-editor-${editingTxId}` : null);
  useMobileEditorScroll(editingRecurringId ? `ledger-recurring-editor-${editingRecurringId}` : null);
    ...
    <div>
      ...
      {activeTab === 'transactions' && (
        <>
          ...
          <section className="grid gap-3 md:hidden" aria-label="流水列表" data-transaction-mobile-list="true">
            ...
            {visibleTransactions.map((transaction: any) => {
              ...
              return (
                <article
                  key={transaction.id}
                  className={`min-w-0 rounded-xl border bg-ledger-surface p-4 ${isEditing ? 'border-ledger-accent/40' : 'border-[var(--border-tertiary)]'}`}
                >
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex min-w-0 items-center gap-2">
                        <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${typeBadgeClass(transaction.type)}`}>{typeLabel(transaction.type)}</span>
                        <span className="truncate text-sm font-medium text-[var(--color-text-primary)]">{transaction.category?.name || '未分类'}</span>
                      </div>
                      <p className="mt-2 truncate text-xs text-ledger-muted">{transaction.description || '无备注'}</p>
                    </div>
                    <div className="shrink-0 text-right">
                      <div className="text-base font-semibold tabular-nums text-[var(--color-text-primary)]">{curSym[transaction.currency || 'CNY'] || '¥'}{transaction.amount}</div>
                      <div className="mt-1 text-xs text-ledger-muted">{new Date(transaction.occurredAt).toLocaleDateString('zh-CN')}</div>
                    </div>
                  </div>

                  <div className="mt-3 grid grid-cols-2 gap-2 rounded-lg bg-ledger-bg/60 p-3 text-xs">
                    <div className="min-w-0">
                      <div className="text-ledger-muted">来源</div>
                      <div className="mt-1 truncate text-[var(--color-text-primary)]">{transaction.fromAsset?.name || '未指定'}</div>
                    </div>
                    <div className="min-w-0">
                      <div className="text-ledger-muted">目标 / 预算</div>
                      <div className="mt-1 truncate text-[var(--color-text-primary)]">{transaction.toAsset?.name || transaction.budget?.name || '—'}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2">
                    <button
                      ...
                      onClick={() => isEditing ? setEditingTxId(null) : startEditTx(transaction)}
                      className="min-h-11 rounded-lg bg-ledger-bg px-3 text-sm font-medium text-[var(--color-text-primary)]"
                    >
                      {isEditing ? '收起编辑' : transaction.fromAccountId || transaction.type !== 'EXPENSE' ? '编辑' : '补来源账户'}
                    </button>
                    <form action={handleDelete}>
                      <input type="hidden" name="id" value={transaction.id} />
                      <button type="submit" className="min-h-11 w-full rounded-lg bg-red-500/10 px-3 text-sm font-medium text-ledger-danger">删除</button>
                    </form>
                  </div>

                  {isEditing ? (
                    <div id={`transaction-editor-${transaction.id}`} className="mt-3 scroll-mt-24 rounded-xl border border-ledger-accent/30 bg-ledger-bg p-3">
                      <MobileEditorBar
                        title="编辑流水"
                        description="保存后同步更新账户与预算"
                        onCancel={() => setEditingTxId(null)}
                        onSave={() => void handleUpdateTransaction(transaction.id)}
                        saveDisabled={!editTxForm.amount.trim()}
                      />
                      <div className="grid grid-cols-2 gap-3">
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">金额</span>
                          <input type="text" inputMode="decimal" value={editTxForm.amount} onChange={(event) => setEditTxForm((previous) => ({ ...previous, amount: event.target.value }))} className={mobileEditorControlClass} />
                        </label>
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">日期</span>
                          <input type="date" value={editTxForm.occurredAt} onChange={(event) => setEditTxForm((previous) => ({ ...previous, occurredAt: event.target.value }))} className={mobileEditorControlClass} />
                        </label>
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">分类</span>
                          <select value={editTxForm.categoryId} onChange={(event) => setEditTxForm((previous) => ({ ...previous, categoryId: event.target.value, budgetId: '' }))} className={mobileEditorControlClass}>
                            <option value="">未分类</option>{categories.map((category: any) => <option key={category.id} value={category.id}>{category.name}</option>)}
                          </select>
                        </label>
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">预算</span>
                          <select value={editTxForm.budgetId} onChange={(event) => setEditTxForm((previous) => ({ ...previous, budgetId: event.target.value }))} className={mobileEditorControlClass}>
                            <option value="">不关联</option>{budgetOptions.map((budget: any) => <option key={budget.id} value={budget.id}>{budget.name}</option>)}
                          </select>
                        </label>
                        <label className="col-span-2 block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">来源资金账户</span>
                          <select value={editTxForm.fromAccountId} onChange={(event) => setEditTxForm((previous) => ({ ...previous, fromAccountId: event.target.value }))} className={mobileEditorControlClass}>
                            <option value="">不指定（只记总收支）</option>{assets.map((asset: any) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}
                          </select>
                        </label>
                        <label className="col-span-2 block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">备注</span>
                          <input type="text" value={editTxForm.description} onChange={(event) => setEditTxForm((previous) => ({ ...previous, description: event.target.value }))} className={mobileEditorControlClass} />
                        </label>
                      </div>
                    </div>
                  ) : null}
          ...
          <div
            ...
            className="hidden rounded-xl bg-ledger-surface overflow-hidden md:block"
          >
      ...
      {activeTab === 'recurring' && (
        <>
                ...
                >
                  <div className="flex min-w-0 items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 text-xs text-ledger-muted">{frequency} · 下次 {new Date(rule.nextDueDate).toLocaleDateString('zh-CN')}</p>
                    </div>
                    <div className="shrink-0 text-right text-base font-semibold tabular-nums text-[var(--color-text-primary)]">{curSym[rule.currency || 'CNY'] || '¥'}{rule.amount}</div>
                  </div>

                  <div className="mt-4 grid grid-cols-3 gap-2">
                    <button type="button" onClick={() => void handleToggleRecurring(rule.id)} className={`min-h-11 rounded-lg px-2 text-sm font-medium ${rule.isActive ? 'bg-green-900/30 text-green-400' : 'bg-ledger-bg text-ledger-muted'}`}>
                      {rule.isActive ? '暂停' : '启用'}
                    </button>
                    <button
                      type="button"
                      aria-expanded={isEditing}
                      aria-controls={`ledger-recurring-editor-${rule.id}`}
                      onClick={() => isEditing ? setEditingRecurringId(null) : startEditRecurring(rule)}
                      className="min-h-11 rounded-lg bg-ledger-bg px-2 text-sm font-medium text-[var(--color-text-primary)]"
                    >
                      {isEditing ? '收起' : '编辑'}
                    </button>
                    <button type="button" onClick={() => void handleDeleteRecurring(rule.id)} className="min-h-11 rounded-lg bg-red-500/10 px-2 text-sm font-medium text-ledger-danger">删除</button>
                  </div>

                  {isEditing ? (
                    <div id={`ledger-recurring-editor-${rule.id}`} className="mt-3 scroll-mt-24 rounded-xl border border-ledger-accent/30 bg-ledger-bg p-3">
                      <MobileEditorBar
                        title={`编辑 ${rule.name}`}
                        description="修改执行规则与下次到期日"
                        onCancel={() => setEditingRecurringId(null)}
                        onSave={() => void handleUpdateRecurring()}
                        saveDisabled={!editRecurringForm.name.trim() || !editRecurringForm.amount.trim()}
                      />
                      <div className="grid grid-cols-2 gap-3">
                        <label className="col-span-2 block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">名称</span>
                          <input type="text" value={editRecurringForm.name} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, name: event.target.value }))} className={mobileEditorControlClass} />
                        </label>
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">金额</span>
                          <input type="text" inputMode="decimal" value={editRecurringForm.amount} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, amount: event.target.value }))} className={mobileEditorControlClass} />
                        </label>
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">分类</span>
                          <select value={editRecurringForm.categoryId} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, categoryId: event.target.value }))} className={mobileEditorControlClass}>
                            <option value="">未分类</option>{categories.map((category: any) => <option key={category.id} value={category.id}>{category.name}</option>)}
                          </select>
                        </label>
                        <label className="col-span-2 block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">来源资金账户</span>
                          <select value={editRecurringForm.fromAccountId} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, fromAccountId: event.target.value }))} className={mobileEditorControlClass}>
                            <option value="">不指定（只记总收支）</option>{assets.map((asset: any) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}
                          </select>
                        </label>
                        <label className="col-span-2 block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">目标资金账户</span>
                          <select value={editRecurringForm.toAccountId} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, toAccountId: event.target.value }))} className={mobileEditorControlClass}>
                            <option value="">不指定</option>{assets.map((asset: any) => <option key={asset.id} value={asset.id}>{asset.name}</option>)}
                          </select>
                        </label>
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">间隔</span>
                          <input type="text" inputMode="numeric" value={editRecurringForm.interval} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, interval: Number(event.target.value) || 1 }))} className={mobileEditorControlClass} />
                        </label>
                        <label className="block min-w-0">
                          <span className="mb-1 block text-xs text-ledger-muted">频率</span>
                          <select value={editRecurringForm.frequency} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, frequency: event.target.value }))} className={mobileEditorControlClass}>
                        ...
                        <label className="col-span-2 block min-w-0">
                          ...
                          <input type="date" value={editRecurringForm.nextDueDate} onChange={(event) => setEditRecurringForm((previous) => ({ ...previous, nextDueDate: event.target.value }))} className={mobileEditorControlClass} />
                        </label>
                      </div>
                    </div>
                  ) : null}
          ...
          {/* Desktop recurring rules list */}
          <div className="hidden rounded-xl bg-ledger-surface overflow-hidden md:block">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ledger-bg text-left text-ledger-muted">
