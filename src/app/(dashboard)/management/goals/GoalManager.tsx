...
export default function GoalManager() {
  ...
  useMobileEditorScroll(editingId ? `goal-editor-${editingId}` : null);
        ...
        <div className="grid gap-3">
          {goals.map((goal) => {
            ...
            return (
              <article
                key={goal.id}
                id={isEditing ? `goal-editor-${goal.id}` : undefined}
                className={`scroll-mt-24 rounded-xl bg-ledger-surface p-4 ${isEditing ? 'ring-1 ring-ledger-accent/40' : ''}`}
              >
                {isEditing ? (
                  <MobileEditorBar
                    title={`编辑 ${goal.name}`}
                    description="保存后立即更新目标进度"
                    onCancel={() => setEditingId(null)}
                    onSave={handleUpdate}
                    saveDisabled={!editForm.name.trim() || !editForm.targetAmount.trim()}
                  />
                ) : null}
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      ...
                      <div className="grid gap-3 md:grid-cols-[minmax(180px,1fr)_80px_64px]">
                        <label className="block">
                          ...
                          <input
                            ...
                            className="min-h-11 w-full rounded-md border border-ledger-bg bg-ledger-bg px-3 py-2 text-sm focus:border-ledger-accent focus:outline-none md:min-h-0"
                          />
                        </label>
                        <label className="block">
                        ...
                        <label className="block">
                          ...
                          <input
                            ...
                            className="h-11 w-full rounded-md border border-ledger-bg bg-ledger-bg px-2 py-1.5 focus:outline-none md:h-[38px]"
                          />
                        </label>
                      </div>
                    ) : (
                  ...
                  <div className={`shrink-0 ${isEditing ? 'hidden items-center gap-3 md:flex' : 'grid w-full grid-cols-2 gap-2 md:flex md:w-auto md:items-center md:gap-3'}`}>
                    {isEditing ? (
                      <>
                      ...
                      <>
                        <button type="button" onClick={() => startEdit(goal)} className="min-h-11 rounded-lg bg-ledger-bg px-3 text-sm font-medium text-ledger-accent md:min-h-0 md:bg-transparent md:px-0 md:hover:underline">编辑</button>
                        <form action={handleDelete} className="min-w-0">
                          <input type="hidden" name="id" value={goal.id} />
                          <button type="submit" className="min-h-11 w-full rounded-lg bg-red-500/10 px-3 text-sm text-ledger-danger md:min-h-0 md:w-auto md:bg-transparent md:px-0 md:hover:underline">删除</button>
                        </form>
                      </>
                    )}
                  </div>
