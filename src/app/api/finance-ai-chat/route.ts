...
export async function POST(request: Request) {
  ...
  try {
    ...
    const input = parseFinanceAiChatInput(await readJsonBody(request));
    assertAllowedAiEndpoint(normalizeFinanceAiEndpoint(input.config.endpoint));
    const result = await sendFinanceAiChat(input);
    ...
    if (!isInputError && !isProviderError && !isHttpError) {
      console.error('[finance-ai-chat] Request failed', error);
    }
