...
- 当前支出分类以 `餐饮、房租、交通、医疗、固定支出、提升品质、旅行、人情往来` 为起点；首屏显示前 7 项，第 8 项固定为“更多”，`人情往来` 从“更多”进入。
- 当前收入分类以 `工资、理财收益` 为起点；分类不足 7 项时按实际数量展示，并在末尾保留“更多”。
...
1. 点击底部“记一笔”进入分类优先界面。
2. 支出为默认类型，首屏分类来自当前账本已有分类，最多显示 7 个分类和 1 个“更多”，且无换行裁切。
3. 用户可以选择分类、输入金额并在不展开更多选项的情况下完成记账。
4. 最近分类、最近账户和今天日期正确默认。
5. `+ / −` 金额计算结果正确，非法金额不能提交。
6. “说一句记账”解析后留在同一快速界面，不跳回完整表单。
7. 成功后显示明确反馈，新流水出现。
8. 失败后数据保留，允许重试。
...
- 单元测试：现有分类筛选、起始顺序、显示上限、金额表达式、默认值和反馈文案。
